...
_p(ДРАКОН) = p(ДРАК) * p(РАКО) * p(АКОН)_
...
типа _float_. По этой причине мы будем использовать не сами вероятности p, а их логарифм _log(p),_ тогда сами значения
будут в пределах ~от -50 до 0. 

Вспомним их курса школьной математики, что _log(a*b) = log(a) + log(b)_ . Получим: 

_log(p("ДРАКОН")) = log(p("ДРАК") * p("РАКО") * p("АКОН"))_

_log(p("ДРАКОН")) = log(p("ДРАК")) + log(p("РАКО")) + log(p("АКОН"))_
